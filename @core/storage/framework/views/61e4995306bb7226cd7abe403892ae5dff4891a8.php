               ...
               * ------------------------------------------*/
                ...
                * ----------------------------------------*/
                ...
                $('body').on('click', '.widget_save_change_button', function (e) {
                    e.preventDefault();
                    var button = $(this);
                    var formContainer = button.closest('form');
                    
                    if (!formContainer.length) {
                        formContainer = button.parent();
                    }
                    
                    var parent = formContainer.find('.widget_save_change_button');
                    var originalText = parent.text();
                    parent.text('Saving...').attr('disabled', true);
                    
                    var widgetType = formContainer.find('input[name="widget_type"]').val();
                    var formAction = formContainer.attr('action');
                    var udpateId = '';

                    // Check if form has file inputs
                    var hasFiles = formContainer.find('input[type="file"]').length > 0;
                        ...
                        success: function (data) {
                            if (data && data.id) {
                                ...
                                if (widgetType == 'new') {
                                    ...
                                    if (!formContainer.find('input[name="id"]').length) {
                                        formContainer.prepend('<input type="hidden" name="id" value="' + udpateId + '">');
                                    } else {
                                        formContainer.find('input[name="id"]').val(udpateId);
                                    }
                                }
                            }
                            parent.text('Saved!').removeClass('btn-info').addClass('btn-success');
                            setTimeout(function () {
                                parent.text(originalText).attr('disabled', false).removeClass('btn-success').addClass('btn-info');
                            }, 2000);
                        },
                        error: function (xhr, status, error) {
                            console.error('Error saving widget:', error, xhr.responseText);
                            var errorMessage = 'Error saving. Please try again.';
                            ...
                            parent.text('Error!').removeClass('btn-info').addClass('btn-danger');
                            setTimeout(function () {
                                parent.text(originalText).attr('disabled', false).removeClass('btn-danger').addClass('btn-info');
                            }, 3000);
                            alert(errorMessage);
                        }
                    };
                    ...
                    if (hasFiles) {
                        ...
                        ajaxData = new FormData(formContainer[0]);
                        ajaxOptions.data = ajaxData;
                        ajaxOptions.processData = false;
                        ajaxOptions.contentType = false;
                    } else {
                        // Use serialize for regular forms
                        ajaxData = formContainer.serialize();
                        ajaxOptions.data = ajaxData;
                    }
...
<?php echo $__env->make('backend.admin-master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\SyanTeck\@core\resources\views/backend/widgets/widget-index.blade.php ENDPATH**/ ?>