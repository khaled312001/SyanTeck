                ...
                <form id="msform" class="msform">
                    ...
                    <fieldset class="padding-top-80 padding-bottom-100">
                        <div class="form-card successful-card">
                            <h2 class="title-step"> {{ get_static_option('success_title') ?? __('نجح') }}</h2>
                            <a href="{{ route('homepage') }}" class="succcess-icon">
                                <i class="las la-check"></i>
                            </a>
                            <h5 class="purple-text text-center">{{ get_static_option('success_subtitle') ?? __('تم إكمال طلبك بنجاح') }}</h5>
                            <div class="btn-wrapper margin-top-35">
                                <h4 class="mb-3">{{ get_static_option('success_details_title') ?? __('تفاصيل طلبك') }}</h4>
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            <th>{{ __('التاريخ والجدول') }}</th>
                                            <th>{{ __('حالة الطلب') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <label><strong>{{ __('حالة الطلب:') }}</strong>
                                                    @if ($order_details->status == 0) <span>{{ __('قيد الانتظار') }}</span>@endif
                                                    @if ($order_details->status == 1) <span>{{ __('نشط') }}</span>@endif
                                                    @if ($order_details->status == 2) <span>{{ __('مكتمل') }}</span>@endif
                                                    @if ($order_details->status == 3) <span>{{ __('تم التسليم') }}</span>@endif
                                                    @if ($order_details->status == 4) <span>{{ __('ملغي') }}</span>@endif
                                                </label>
                                            </td>
                                        </tr>
                                    </tbody>
                            ...
                            <div class="btn-wrapper text-center margin-top-35">
                                <a href="{{ route('homepage') }}" class="cmn-btn btn-bg-1">{{ get_static_option('button_title') ?? __('العودة للرئيسية') }}</a>
                                @if(auth('web')->check())
                                    @php
                                    $user_details = auth('web')->user();
                                    $route_prefix = $user_details->user_type === 0 ? 'seller' : 'buyer';
                                    @endphp
                                    <a href="{{ route($route_prefix.'.dashboard') }}" class="cmn-btn btn-bg-1">{{__('الذهاب للوحة التحكم') }}</a>
                                    <a href="{{ route($route_prefix.'.orders') }}" class="cmn-btn btn-bg-1">{{__('جميع الطلبات') }}</a>
        ...
        ;(function(exports) {
            ...
            var calendarGenerators = {
                google: function(event) {
                    ...
                    var calendarText = "{{__('Add To Google Calendar')}}";
