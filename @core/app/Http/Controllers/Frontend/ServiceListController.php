...
class ServiceListController extends Controller
{
    ...
    public function serviceBook($slug)
    {
        ...
        // Get all active regions (like in QR page)
        $regions = Region::where('is_active', true)->orderBy('name_ar', 'asc')->get();
    ...
    public function scheduleByDay(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!is_null($day)) {
                ...
                if (is_null($same_schedule_allow_or_not)){
                    return response()->json([
                        'status' => 'no schedule',
                    ]);
                }
    ...
    public function createOrder(Request $request)
    {
        ...
        if (empty($request->service_subtotal_input_hidden_field_for_js_calculation)) {
            toastr()->error(__('Please choose Include or Additional under Service.'));
            return back();
        }
        ...
        // Only validate manual payment image if manual_payment is selected
        if($request->selected_payment_gateway === 'manual_payment') {
            $this->validate($request,[
                'manual_payment_image' => 'required|mimes:jpg,jpeg,png,pdf,webp'
            ]);
        }
        // For cash_on_delivery, no validation needed
        ...
        // Always create support ticket for new orders
        ...
        if (isset($request->services) && is_array($request->services)) {
            ...
            if($request->is_service_online_ != 1) {
                foreach ($request->services as $key => $requested_service) {
                    //todo:: Check if service main include  is not null or empty
        ...
        if($commission->system_type == 'subscription'){
            if(subscriptionModuleExistsAndEnable('Subscription')){
                $commission_amount = 0;
                \Modules\Subscription\Entities\SellerSubscription::where('seller_id', $request->seller_id)->update([
        ...
        // Send notification to support team and admins
        try {
            ...
            foreach ($supportUsers as $user) {
                $user->notify(new OrderNotification($last_order_id, $request->service_id, $request->seller_id, $buyer_id, __('New order received - Order #:id', ['id' => $last_order_id])));
            }
        } catch (\Exception $e) {
            \Log::error('Error sending support team notifications: ' . $e->getMessage());
        }
            ...
            elseif($request->selected_payment_gateway === 'paytabs'){
                ...
                try{
                    ...
                    $redirect_url = $kineticpay->charge_customer([
                        ...
                        'title' => $title,
                        'description' => $description,
                        'ipn_url' => route('frontend.kineticpay.ipn'),
                        'order_id' => $last_order_id,
                        'track' => \Str::random(36),
                        'cancel_url' => route(self::CANCEL_ROUTE,$last_order_id),
                        'success_url' => route(self::SUCCESS_ROUTE,$new_order_id),
    ...
    {
        $all_featurd_service = Service::select('id','title','image','description','price','slug','seller_id', 'service_city_id', 'is_service_online')
            ->with('reviews')
    ...
    {
        $all_popular_service = Service::select('id','title','image','description','price','slug','seller_id','view','featured', 'service_city_id', 'is_service_online')
            ->with('reviews')
    ...
    {
        $all_category = Category::select('id','name','slug','image', 'status')->where('status', 1)->with('services')->get();
