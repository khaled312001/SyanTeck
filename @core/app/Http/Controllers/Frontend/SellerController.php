...
//            event(new SupportMessage($ticket_info));
    ...
    /**
     * إرسال تقرير الفني مع الصور والفيديوهات
     */
    public function submitTechnicianReport(Request $request, $orderId)
    {
        ...
        $request->validate([
            ...
            'technician_videos.*' => 'nullable|mimes:mp4,avi,mov,wmv|max:51200', // 50MB max per video
        ], [
            'technician_report.required' => __('يجب كتابة تقرير الصيانة'),
            'technician_images.*.image' => __('يجب أن تكون الملفات المرفوعة صور'),
            'technician_images.*.max' => __('حجم الصورة يجب أن يكون أقل من 5 ميجابايت'),
            'technician_videos.*.mimes' => __('يجب أن تكون الفيديوهات بصيغة mp4, avi, mov, wmv'),
            'technician_videos.*.max' => __('حجم الفيديو يجب أن يكون أقل من 50 ميجابايت'),
        ]);
        ...
        // رفع الصور
        if ($request->hasFile('technician_images')) {
            foreach ($request->file('technician_images') as $image) {
                $imageName = time() . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
                $image->move(public_path('assets/uploads/technician-reports/images/'), $imageName);
                $images[] = 'assets/uploads/technician-reports/images/' . $imageName;
            }
        }

        // رفع الفيديوهات
        if ($request->hasFile('technician_videos')) {
            foreach ($request->file('technician_videos') as $video) {
                $videoName = time() . '_' . uniqid() . '.' . $video->getClientOriginalExtension();
                $video->move(public_path('assets/uploads/technician-reports/videos/'), $videoName);
                $videos[] = 'assets/uploads/technician-reports/videos/' . $videoName;
            }
        }
        ...
        $allVideos = array_merge($existingVideos, $videos);

        // تحديث الطلب
        $order->update([
            'technician_report' => $request->technician_report,
            'technician_images' => $allImages,
            'technician_videos' => $allVideos,
            'technician_report_submitted_at' => now(),
        ]);

        // إرسال إشعار للإدمن وفريق الدعم
        // يمكن إضافة إشعارات هنا

        toastr_success(__('تم إرسال التقرير بنجاح'));
        return redirect()->back();
    }
