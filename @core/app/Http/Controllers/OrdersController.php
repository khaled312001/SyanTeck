...
//            event(new SupportMessage($ticket_info));
    ...
    /**
     * تسعير الخدمة من قبل الإدمن/فريق الدعم
     */
    public function setAdminPricing(Request $request, $orderId)
    {
        $order = Order::findOrFail($orderId);

        $request->validate([
            'admin_pricing' => 'required|numeric|min:0',
            'admin_pricing_notes' => 'nullable|string|max:1000',
        ], [
            'admin_pricing.required' => __('يجب إدخال السعر'),
            'admin_pricing.numeric' => __('السعر يجب أن يكون رقماً'),
            'admin_pricing.min' => __('السعر يجب أن يكون أكبر من أو يساوي صفر'),
            'admin_pricing_notes.max' => __('ملاحظات التسعير يجب أن تكون أقل من 1000 حرف'),
        ]);

        $order->update([
            'admin_pricing' => $request->admin_pricing,
            'admin_pricing_notes' => $request->admin_pricing_notes,
            'admin_pricing_at' => now(),
            'admin_pricing_by' => auth()->guard('admin')->id(),
        ]);

        // تحديث السعر الإجمالي للطلب
        $order->total = $request->admin_pricing;
        $order->save();

        // إرسال إشعار للفني والعميل
        // يمكن إضافة إشعارات هنا

        return redirect()->back()->with(FlashMsg::item_new(__('تم تسعير الخدمة بنجاح')));
    }
}
