...
class ServiceController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (empty(optional(optional($row->seller)->account_status)->account_status) || optional(optional($row->seller)->account_status)->account_status == 0){
                            if ($admin->can('service-edit') || $admin->can('admin-service-edit')) {
                                $action .= General::editIcon(route('admin.edit.service', $row->id));
                            }
    ...
    public  function sellerServiceCoupon(){
        $seller_service_coupons = ServiceCoupon::where('user_type', null)->latest()->get();
        return view('backend.pages.services.seller-service-coupons',compact('seller_service_coupons'));
    }

    public function seller_change_coupon_status($id=null)
    {
    ...
    public function seller_delete_coupon($id = null)
    {
    ...
    public function service_load_more_category_settings()
    {
        return view('backend.pages.services.service-load-more-category-settings');
    }

    public function load_more_category_settings_update(Request $request)
    {
        $this->validate($request, [
            'load_more_button_show_hide_settings' => 'nullable'
        ]);

        $all_fields = [
            'load_more_button_show_hide_settings'
        ];
    ...
    public function service_zone_settings()
    {
        return view('backend.pages.services.service-zone-settings');
    }

    public function service_zone_settings_update(Request $request)
    {
        $request->validate([
            'service_google_map_api_key' => 'nullable',
            'google_map_settings' => 'nullable',
            'google_map_search_placeholder_title' => 'nullable',
            'google_map_search_button_title' => 'nullable'
        ]);

        update_static_option('service_google_map_api_key',$request->service_google_map_api_key);
        update_static_option('google_map_settings',$request->google_map_settings);
        ...
        update_static_option('select_home_page_search_service_page_url',$request->select_home_page_search_service_page_url);
        return redirect()->back()->with(FlashMsg::item_new('Update Success'));
    ...
    {
        if ($request->isMethod('post')) {
            // Custom validation for description (check text length without HTML tags)
            $request->validate([
                'category' => 'required',
                ...
                'description' => [
                    ...
                    function ($attribute, $value, $fail) {
                        ...
                        $textLength = mb_strlen($textOnly, 'UTF-8');
                        if ($textLength < 150) {
                            $fail(__('The description must be at least 150 characters.'));
                        }
                    },
                ],
                'slug' => 'required',
...
//            return redirect()->back()->with(FlashMsg::item_new('Service Added Success'));
    ...
    public function edit_service(Request $request, $id)
    {
        if ($request->isMethod('post')) {
            ...
            Service::where('id', $id)->update([
                ...
                'is_service_online' => $request->is_service_online ?? 0,
                'online_service_price' => $request->online_service_price ?? 0,
            ...
            return redirect()->route('admin.services')->with(FlashMsg::item_new('Service Updated Success'));
        }
    ...
    public function add_service_attributes(Request $request)
    {
        if($request->isMethod('post')){
            ...
            return redirect()->route('admin.services')->with(FlashMsg::item_new('Service Attributes Added Success'));
        }
    ...
    public function edit_service_attributes_by_id(Request $request, $id = null)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->route('admin.services')->with(FlashMsg::item_new('Service Attributes Updated Success'));
        }
    ...
    public function show_service_attributes_by_id($id=null)
{
    ...
    // Update all services with missing data
    public function updateAllServicesData(Request $request)
    {
        $forceUpdate = $request->get('force', false); // خيار لتحديث جميع الخدمات بالقوة
        ...
        foreach ($services as $index => $service) {
            ...
            // تحديث الوصف إذا كان قصيراً (أقل من 150 حرف) أو فارغاً أو إذا كان force update
        ...
        return redirect()->back()->with(FlashMsg::item_new("تم تحديث {$updated} خدمة من أصل {$totalServices} خدمة بالبيانات الوهمية الكاملة. تم تخطي {$skipped} خدمة."));
    }

    // Update single service with fake data
    public function updateSingleServiceData($id)
    {
        // قائمة الأوصاف الوهمية (كل وصف أكثر من 150 حرف)
        $fakeDescriptions = [
            '<p>نوفر خدمات صيانة وإصلاح شاملة ومتكاملة لجميع احتياجاتك اليومية. فريقنا من الفنيين المعتمدين والمحترفين جاهز لتقديم أفضل الخدمات بأعلى معايير الجودة والسلامة. نحن نضمن لك خدمة سريعة وفعالة مع ضمان شامل على جميع الأعمال المنجزة. خبرتنا الواسعة في المجال تمكننا من التعامل مع جميع أنواع الأعطال والمشاكل بسرعة وكفاءة عالية.</p><p><br></p><p>تشمل خدماتنا:</p><ul><li>فحص شامل وتشخيص دقيق للمشكلة</li><li>إصلاح وصيانة احترافية بجودة عالية</li><li>استبدال القطع التالفة بقطع أصلية وموثوقة</li><li>ضمان شامل على جميع الأعمال المنجزة</li><li>خدمة عملاء على مدار الساعة طوال أيام الأسبوع</li><li>أسعار شفافة ومنافسة في السوق</li><li>استخدام أحدث المعدات والتقنيات الحديثة</li><li>فريق محترف ومدرب على أعلى المستويات</li></ul>',
        ...
        // تحديث الوصف دائماً لضمان أنه أكثر من 150 حرف
        $currentDescription = strip_tags($service->description ?? '');
        $currentDescriptionLength = mb_strlen($currentDescription, 'UTF-8');
        // تحديث الوصف إذا كان فارغاً أو أقل من 150 حرف
        ...
        // تحديث نوع الخدمة إذا كان null
        if (is_null($service->is_service_online)) {
            $updateData['is_service_online'] = rand(0, 1);
        }
        ...
        // تحديث is_service_on إذا كان null
        if (is_null($service->is_service_on)) {
            $updateData['is_service_on'] = 1;
        }

        // تحديث featured إذا كان null
        if (is_null($service->featured)) {
            $updateData['featured'] = rand(0, 1);
        }

        // تحديث view إذا كان null
        if (is_null($service->view)) {
            $updateData['view'] = rand(0, 1000);
        }

        if (!empty($updateData)) {
            Service::where('id', $id)->update($updateData);
            return redirect()->route('admin.edit.service', $id)->with(FlashMsg::item_new("تم تحديث الخدمة رقم {$id} بنجاح بالبيانات الوهمية الكاملة."));
        }

        return redirect()->route('admin.edit.service', $id)->with(FlashMsg::error("الخدمة رقم {$id} لا تحتاج تحديث."));
    }
}
