...
class PaymentController extends Controller
{
    ...
    public function send_order_mail($order_id)
    {
        ...
        try {
            ...
            Mail::to($order_details->email)->send(new OrderMail($mail_subject.$order_details->id,$order_details, $message_for_buyer));
            Mail::to($seller_email->email)->send(new OrderMail($mail_subject.$order_details->id,$order_details, $message_for_seller_admin));
            Mail::to(get_static_option('site_global_email'))->send(new OrderMail($mail_subject.$order_details->id,$order_details, $message_for_seller_admin));
