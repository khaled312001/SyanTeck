...
function render_image_markup_by_attachment_id($id, $class = null, $size = 'full')
{
    ...
    if (empty($id)){
        $noImageMarkupRender =  asset('assets/uploads/no-image.png');
        $on_image_output_return = '<img src="'.$noImageMarkupRender.'" class="no-image" alt="no-image"/>';
        return $on_image_output_return;
    }
    ...
    $noImageMarkupRender =  asset('assets/uploads/no-image.png');
     ...
     if (!empty($image_details)) {
             if(empty($image_details['img_url'])){
                $output = '<img src="'.$noImageMarkupRender.'" class="no-image" alt="' . ($image_details['img_alt'] ?? "") . '"/>';
                 return $output;
             }
         $output = '<img src="' . $image_details['img_url'] . '" ' . $class_list . ' alt="' . ($image_details['img_alt'] ?? "") . '"/>';
     }else{

         $output = '<img src="'.$noImageMarkupRender.'" class="no-image" alt="no-image"/>';
     }
...
function render_payment_gateway_for_form()
{
    ...
    $all_gateway = ['paypal','manual_payment','mollie','paytm','stripe','razorpay','flutterwave','paystack','marcadopago','instamojo','cashfree','payfast','midtrans','squareup','cinetpay','paytabs','billplz','zitopay', 'kineticpay'];
...
function render_drag_drop_form_builder_field_markup($key, $type, $name, $placeholder, $required, $args = [])
{
    ...
    if ($type == 'file') {
        ...
        $output .= '<option value="mimes:jpg,jpeg,png,webp"';
        if (isset($args['mimes_type']) && $args['mimes_type'] == 'mimes:jpg,jpeg,png,webp') {
            $output .= "selected";
        }
        $output .= '>' . __('mimes:jpg,jpeg,png,webp') . '</option>';
...
function sellerServiceLocation($service){
    ...
    if (!empty(get_static_option('google_map_settings'))){
            if ($service->is_service_online == 1){
                $seller_city_and_country = __('Online Service');
                return $seller_city_and_country;
            }else{
                // if google map active show seller set Google map in pick address
                $seller_city_and_country =  optional($service->seller)->seller_address ?? '';
                return $seller_city_and_country;
           }
    }else{
        if ($service->is_service_online == 1){
            ...
            if(!empty($service->service_city_id)) {
                $seller_city_name = optional(optional($service)->serviceCity)->service_city;
                $seller_country_name = optional(optional(optional($service)->serviceCity)->countryy)->country;
                // check if seller service city is null
                if (!empty($seller_city_name)) {
                    $seller_city_and_country = $seller_city_name . ', ' . $seller_country_name ?? '';
                } else {
                    $seller_city_and_country = $seller_country_name ?? '';
                }
                return $seller_city_and_country;
            }else{
                $seller_city_and_country =  '';
                return $seller_city_and_country;
            }
        }
    }
...
function sellerProfileLocation($seller){
    if (empty($seller)){
        return '';
    }
    if (!empty(get_static_option('google_map_settings'))){
            $seller_city_and_country =  $seller->seller_address ?? '';
            return $seller_city_and_country;
    }else{
        $seller_city_name = optional(optional($seller)->city)->service_city;
        $seller_country_name = optional($seller->country)->country;
        $seller_city_and_country = $seller_city_name . ', ' . $seller_country_name ?? '';
        return $seller_city_and_country;
    }
}
